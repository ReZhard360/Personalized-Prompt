...
# Istruzioni base per il narratore (versioni per ciascuna lingua)
...
# Template APE per la generazione di prompt (incluso un riferimento ispirato al paper [2402.00284])
...
def compute_bart_score(candidate: str, reference: str) -> float:
    # Placeholder per BARTScore: integrare il wrapper BARTScore se disponibile
    return random.uniform(0, 1)


def compute_mover_score(candidate: str, reference: str) -> float:
    # Placeholder per MoverScore: integrare il pacchetto moverscore_v2 se disponibile
    return random.uniform(0, 1)
...
def generate_series_personalized_summary(monument: Dict[str, Any], age_group: str, interest: str,
    ...
    # Salva tutte le combinazioni in un CSV
...
def process_query(selected_monument_name: str, age_group: str, interest: str, narrator_type: str,
                  language: str, temperature: float, selected_model: str, chunk_size: int, overlap_size: int,
    ...
    if retrieval_method == "ElasticSearch":
        ...
        # Prepara una stringa Markdown con i dettagli APE per la visualizzazione
...
with gr.Blocks() as demo:
    ...
    with gr.Row():
        ...
        model_dropdown = gr.Dropdown(
            label="(Admin) Seleziona modello LLM per test",
            choices=["llama3.2", "llama3.3", "qwen2.5", "llama3.1", "mistral", "deepseek-r1"],
            value="llama3.2",
            allow_custom_value=True
        )
    ...
    def compare_models(selected_monument, query, age_grp, interest_val,
                       narrator_val, language_val, temp, model_choice,
        ...
        results = [[model_choice, mon_name, summary, tokens, eval_metrics]]
    ...
    btn_generate.click(
        ...
        inputs=[monument_dropdown, query_input, age_group, interest,
                ...
                model_dropdown, chunk_slider, overlap_slider,
