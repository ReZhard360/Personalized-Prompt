...
# Nuove importazioni per il soft prompt generator
import torch
...
########################################
# INTEGRAZIONE DEI CONCETTI PPR: FUNZIONI DI AUGMENTATION E SOFT PROMPT
########################################
def prompt_based_augmentation(prompt: str) -> List[str]:
    """
    Genera varianti del prompt originale per aumentare la robustezza.
    In un'implementazione più avanzata si potrebbero usare tecniche di NLP per sostituzioni sinonimiche.
    """
    augmented_prompts = []
    augmented_prompts.append(prompt + " [Augmented Version 1]")
    augmented_prompts.append("Nota: " + prompt)
    augmented_prompts.append(prompt.replace("monumento", "luogo storico"))
    return augmented_prompts

def generate_soft_prompt(user_profile: Dict[str, Any]) -> str:
    """
    Dummy implementation di un Soft Prompt Generator.
    In una versione avanzata, si potrebbe utilizzare un MLP addestrato per generare un prompt soft in base al profilo utente.
    """
    attributes = [
        user_profile.get("age_group", ""),
        user_profile.get("interest", ""),
        user_profile.get("narrator_type", "")
    ]
    combined = " ".join(attributes)
    return f"Soft Prompt: {combined}"
...
def generate_series_personalized_summary(monument: Dict[str, Any], age_group: str, interest: str,
    ...
    integrando tecniche di prompt augmentation e soft prompt generation per un livello di personalizzazione elevato.
    Per ogni template (in base alla lingua), vengono generate diverse varianti del prompt (originale, augmentata e con soft prompt)
    e viene selezionato il riassunto migliore in base alle metriche di valutazione.
    """
    if language == "English":
    ...
    # Preparo il profilo utente per il soft prompt
    user_profile = {
        "age_group": age_group,
        "interest": interest,
        "narrator_type": narrator_type
    }
    soft_prompt = generate_soft_prompt(user_profile)
    ...
    # Per ogni template generiamo varie candidate (base, augmented e con soft prompt)
    for template in templates:
        ...
        # Genera varianti del prompt tramite augmentation
        augmented_prompts = prompt_based_augmentation(base_prompt)
        # Combina: prompt base, le varianti e una versione con il soft prompt appeso
        candidate_prompts = [base_prompt] + augmented_prompts + [base_prompt + " " + soft_prompt]
        for candidate in candidate_prompts:
            logging.info(f"Candidate Prompt: {candidate}")
            llm.temperature = temperature
            try:
                result = llm.generate([candidate])
                summary = result.generations[0][0].text.strip()
            except Exception as e:
...
with gr.Blocks() as demo:
    ...
    with gr.Row():
        ...
        # Il dropdown per il modello è stato rimosso; verrà usato sempre DEFAULT_MODEL.
